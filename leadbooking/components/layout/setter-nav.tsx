...
const navItems = [
  { href: '/setter', label: 'Dashboard', icon: LayoutDashboard, exact: true },
  { href: '/setter/termin-anlegen', label: 'Termin anlegen', icon: PlusCircle },
  { href: '/setter/termine', label: 'Meine Termine', icon: List },
]
    ...
    <>
      <aside className="hidden lg:flex flex-col w-64 min-h-screen bg-[#1E3A5F] text-white fixed left-0 top-0 bottom-0 z-30">
        <div className="p-6 border-b border-white/10">
          <Logo light />
          <p className="text-white/60 text-xs mt-1">Setter-Portal</p>
        </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-20 bg-[#1E3A5F] text-white pt-16">
          <nav className="px-4 py-6 space-y-1">
            {navItems.map(({ href, label, icon: Icon, exact }) => (
              <Link
                ...
                className={cn(
                  ...
                  isActive(href, exact) ? 'bg-white/20 text-white' : 'text-white/70'
                )}
              >
            ...
            <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium text-white/70 w-full">
              <LogOut className="w-5 h-5" />
              Abmelden
            </button>
          </nav>
