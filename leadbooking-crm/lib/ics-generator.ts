...
export function generateIcs(input: IcsInput): string {
  ...
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Hebammen-Vorsorge//CRM//DE',
    'METHOD:REQUEST',
    'CALSCALE:GREGORIAN',
    'BEGIN:VEVENT',
    `UID:${input.leadId}@hebammen-vorsorge.de`,
    `DTSTAMP:${fmtUtc(now)}`,
    ...
    `ORGANIZER;CN=${escape(input.setterName)}:mailto:${input.setterEmail ?? 'beratung@hebammen-vorsorge.de'}`,
