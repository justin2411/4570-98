...
export function SetterManagement({ initialSetters }: { initialSetters: Profile[] }) {
  ...
  async function createSetter(e: React.FormEvent) {
    ...
    if (json.profile) setSetters(prev => [...prev, json.profile])
    setShowCreate(false)
  ...
  async function deleteSetter(setter: Profile) {
    if (!setter?.id) return
    if (!confirm('Setter wirklich löschen?')) return
    const res = await fetch('/api/admin/delete-setter', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: setter.id }),
    })
    if (res.ok) { setSetters(prev => prev.filter(s => s?.id !== setter.id)); toast.success('Setter gelöscht') }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-700 text-sm mt-1">{setters.filter(s => s != null).length} Setter</p>
        </div>
      ...
      {showCreate && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900">Neuen Setter anlegen</h2>
            <button onClick={() => setShowCreate(false)}><X className="w-5 h-5 text-gray-700" /></button>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 hidden md:table-cell">E-Mail</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 hidden lg:table-cell">Erstellt</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700">Status</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700">Aktionen</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {setters.filter(s => s != null).map(s => (
              <tr key={s.id} className={s.is_active === false ? 'opacity-50' : ''}>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <Avatar name={s.full_name ?? ''} color={s.avatar_color ?? '#2E75B6'} size="sm" />
                    <span className="font-medium text-gray-900">{s.full_name ?? '–'}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-gray-800 hidden md:table-cell">{s.email ?? '–'}</td>
                <td className="px-4 py-3 text-gray-700 hidden lg:table-cell">{formatDate(s.created_at ?? '')}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => resetPassword(s.email ?? '')} className="text-xs text-gray-700 hover:underline">
                      Passwort reset
                    </button>
                    <button onClick={() => deleteSetter(s)} className="text-xs text-red-600 hover:underline">
                      Löschen
                    </button>
                  </div>
                </td>
        ...
        {setters.filter(s => s != null).length === 0 && <p className="text-center py-10 text-gray-700">Keine Setter</p>}
      </div>
    </div>
  )
}
