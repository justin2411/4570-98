...
## D-019 · Persistente Blacklist für `kein_interesse`

**Entscheidung:** Jeder Lead, der auf `kein_interesse` wechselt, landet automatisch auf einer zentralen `blacklist`-Tabelle (Key = normalisierte Telefonnummer). Die Blacklist überlebt das Löschen des Lead-Datensatzes, und re-importierte Telefonnummern werden beim Insert sofort auf `kein_interesse` gesetzt — sie tauchen nirgends mehr im aktiven Pool auf.

**Warum:** „Wir rufen niemanden zweimal an, dem es nicht passt." Frühere Implementierung verlässt sich nur auf `status='kein_interesse'` — sobald der Lead per Hard-Delete oder Excel-Reimport wieder neu in die Tabelle kommt, wäre er als „neu" wieder im Pool. Die Blacklist macht die Ablehnung **persistent über den Lead-Lebenszyklus hinaus**.

**Umsetzung:**
- Tabelle `blacklist` (id, phone UNIQUE normalisiert, email, name, beruf, original_lead_id mit ON DELETE SET NULL, reason, created_at, created_by).
- DB-Trigger `AFTER INSERT OR UPDATE OF status ON leads` → idempotenter INSERT in `blacklist` bei `status='kein_interesse'`.
- DB-Trigger `BEFORE INSERT ON leads` → wenn Phone in Blacklist, NEW.status := 'kein_interesse' (Defense in Depth gegen Re-Import).
...
- SQL: `supabase/blacklist-setup.sql` — idempotent, einmal in Supabase einspielen.
