...
## D-022 · Cockpit-Deck startet leer — Setter wählt aktiv eine Zielgruppe

**Entscheidung:** Beim ersten Öffnen des Cockpits ist der Deck **leer**. Über die Chip-Reihe oben wählt der Setter explizit einen Beruf-Filter (z. B. „Physiotherapeut"), den „Nur Handys"-Filter oder den High-Potential-Tab. Ohne explizite Wahl wird kein Lead geladen.

**Warum:** Vorher wurde ein "Default-Beruf" (der häufigste in der Setter-Liste) automatisch geladen. Das hat Setter dazu verleitet, blind den ersten Lead anzurufen, obwohl sie eigentlich eine andere Zielgruppe abarbeiten wollten. Bewusste Wahl = bewusste Telefonate. Außerdem: bei Reload kommt der Setter genau dort hin, wo er war (Beruf bleibt in der URL-Query).

**Konsequenz:** Beruf-Switch im Cockpit forciert einen vollen Page-Remount (`key={beruf}` auf `CockpitClient`) — ansonsten würde `useState(initialDeck)` den alten State behalten und das Switching wäre wirkungslos.

---

## D-023 · „Nur Handys"-Filter + Handys-First-Sortierung

**Entscheidung:** Setter haben einen Toggle „📱 Nur Handys" im Cockpit/Lead-Liste, der den Deck strikt auf Handynummern (`+49 15x/16x/17x`) reduziert. Unabhängig davon ist die Sortier-Reihenfolge in BEIDEN Listen jetzt: nie angerufene → **Handys zuerst** → Probability-Score → Lead-Score.

**Warum:** Handynummern haben höhere Connect-Rate und sind das einzige zuverlässige Signal für „erreichbar". Setter-Feedback: „mit Handys mache ich 3x so viele Termine wie mit Festnetz". Statt zwei separater UIs (Handys / alle) bleibt eine UI mit Filter — Handys sind aber immer am Anfang sichtbar.

**Wo:** Cockpit-Deck (`/setter/cockpit`), Setter-Lead-Liste (`/setter/leads`). Implementierung via Helper `isHandyLead(lead)` in `lib/handy-check.ts`.

---

## D-024 · `prio_a`-Flag + High-Potential-Tab im Cockpit

**Entscheidung:** Leads tragen eine boolesche Spalte `prio_a`. Im Cockpit existiert ein eigener Tab „⭐ High Potential (n)", der ausschließlich `prio_a=true`-Leads zeigt — über alle Berufsgruppen hinweg, kein Beruf-Filter wirkt darin. Pflege erfolgt per Admin / per Token-PATCH (`PATCH /api/admin/leads` mit `prio_a` in der Whitelist).

**Warum:** Für Sonder-Aktionen / kuratierte Listen (z. B. „48 Premium-Leads aus dem Vendor-CSV"). Setter sieht den Tab nur, wenn er Leads zugewiesen bekommen hat, die `prio_a=true` sind. Der Tab überschreibt den Beruf-Filter — gewollt, damit die gesamte kuratierte Liste auf einen Blick abarbeitbar ist.

**Mechanik:** `GET /api/admin/leads?prio=true` filtert serverseitig. PATCH-Whitelist enthält `prio_a` (siehe `PATCHABLE_COLUMNS` in `/api/admin/leads/route.ts`).

---

## D-025 · Token-fähiger Bulk-Import (`POST /api/admin/leads/bulk`)

**Entscheidung:** Der Agent kann komplette CSV-Datensätze per JSON-Bulk-Import einspielen, ohne dass der User Excel hochladen muss. Endpoint akzeptiert Bearer-Token, prüft Duplikate über die normalisierte Telefonnummer, dedupliziert gegen die Blacklist und liefert `inserted` + `skipped`-Counts zurück. NOT-NULL-Spalten (`email`, `state`, …) werden mit `''` statt `null` befüllt, damit das DB-Schema nicht crasht.

**Warum:** Excel-Import-UI ist gut für Setter/Admins, aber für agent-getriebene Workflows (z. B. „importier diese 48 Leads und setz sie auf prio_a=true bei Setter X") zu klick-intensiv. Token-Endpoint macht das in einem Call.

**Auth:** Bearer-Token (`ADMIN_API_TOKEN`), gleicher Token wie alle anderen Admin-Calls.

---

## D-026 · Cockpit-Deck-State strikt lokal-synchron zur DB

**Entscheidung:** Nach jeder Terminal-Action im Cockpit (`kein_interesse`, `nicht_erreicht`, `termin_gelegt`, `wiedervorlage`) wird der Lead **sofort aus dem lokalen `deck`-State entfernt**, nicht nur der `currentIdx` weitergerückt. Zusätzlich filtert der `useState`-Initializer beim Mount defensiv auf `status ∈ {neu, angerufen, wiedervorlage}` — selbst wenn der Server-Query (theoretisch durch Stale-Cache / Race) einen Lead mit falschem Status durchlassen würde.

**Warum:** Setter hatten gemeldet, dass nach „App schließen + neu öffnen" bearbeitete Leads (auf nicht_erreicht / kein_interesse gesetzt) gelegentlich wieder im Cockpit auftauchten. Ursache: der lokale `deck`-State enthielt den Lead noch mit altem Status (Karte sichtbar via `goBack()`); zusätzlich gab es kein hartes Frontend-Filter als Sicherung gegen seltsame Server-Antworten. Mit dem Fix ist sowohl Live-State als auch Reload-Verhalten konsistent.

**Konsequenz:** `undoLastAction` musste angepasst werden: `captureSnapshot` speichert jetzt den ganzen `lead` + `indexInDeck`. Undo setzt den Lead an seine alte Position zurück (oder updated ihn, falls noch im Deck — z. B. Wiedervorlage in der gleichen Session).

---

## D-027 · Normalisierte Dublettenerkennung beim Import (über Roh-Phone hinaus)

**Entscheidung:** Excel- **und** Token-Bulk-Import gleichen eingehende Leads über den **normalisierten** Telefon-Key ab (zentrale `normalizePhoneKey()` in `lib/phone.ts`, identisch zur DB-`normalize_phone()`), nicht mehr nur über den rohen `leads.phone`-String. Abgeglichen wird gegen **bestehende Leads** (paginiert) **und** die **Blacklist**; Treffer werden übersprungen, datei-interne Dubletten ebenfalls.

**Warum:** `leads.phone UNIQUE`/`onConflict='phone'` greift nur bei BYTE-identischem String — dieselbe Nummer in anderer Schreibweise (`0151…` / `+49151…` / `0049151…`) landete doppelt im aktiven Pool. Damit war das Versprechen „wer schon mal im System war, wird als Duplikat erkannt" (Termine, Wiedervorlagen, Kein Interesse) verletzt. Zusätzlich lief der Excel-Dublettencheck ohne Pagination und sah nur die ersten ~1000 Leads.

**Abdeckung der drei „Hintergrund"-Kategorien:** Kein Interesse → Blacklist (überlebt Hard-Delete, D-019). Termine/stattgefunden + Wiedervorlagen → per Lösch-Schutz (D-020) als echte Leads erhalten → normalisierter Lead-Abgleich. Wiedervorlagen bleiben **bewusst nicht** in der Blacklist (sonst würde `filterBlacklistedLeads` sie aus dem Cockpit-Deck werfen).

**Status:** Live (Mai 2026, PR #43). Bestätigt am echten Import: von 2.919 Datei-Leads waren nur 384 neu, 2.359 schon vorhanden, 176 Blacklist.

---

## D-028 · Vollständige Pagination gegen den PostgREST-1000-Zeilen-Deckel

**Entscheidung:** Alle Stellen, die den gesamten Lead-Bestand aggregieren/verteilen, laden über den Helper **`fetchAllRows()`** (`lib/supabase/fetch-all.ts`) — paginiert per `.range()` und **stabil nach `id` sortiert**. Betroffen: `lead-probability` (Modell-Training), `distribute-leads` (Verteil-Pool), `stats` (Aggregate). `GET /api/admin/leads` deckelt `limit` ehrlich auf 1000.

**Warum:** PostgREST liefert pro Antwort hart max. 1000 Zeilen. Ein `.select()` ohne `.range()` ignorierte stillschweigend ~7.000 von ~8.000 Leads → verfälschte Statistik, unvollständige Verteilung, ein nur auf 1.000 Leads trainiertes Modell. Live bestätigt: `GET …?limit=2000` lieferte nur 1.000; ohne stabile Sortierung verschoben sich beim Offset-Paging Zeilen (Doppel-/Auslassungen).

**Status:** Live (PR #44).

---

## D-029 · Beruf-balancierte Verteilung + serverseitiger Hebammen-Freeze

**Entscheidung:** `distribute-leads` kann mit **`balanceByBeruf`** pro Beruf-Gruppe round-robin verteilen (fortlaufender Rotations-Zeiger über alle Gruppen) → jeder Setter erhält denselben Beruf-Mix statt nur global reihum. Zusätzlich schließt **`excludeBeruf`** (default `['hebamm%']`) Berufe aus; damit ist der **Hebammen-Freeze** jetzt im Endpoint erzwungen (vorher nur prozedural). Per `excludeBeruf: []` abschaltbar. UI: Checkbox „Pro Beruf gleichmäßig verteilen" im Verteilen-Dialog (default an).
